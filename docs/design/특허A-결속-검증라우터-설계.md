...
- **T3 — WebAuthn 검증 경로(COSE 실검증)**
  `WebAuthnVerificationPath` + webauthn4j 연동. SAM `WebAuthnService.verify()` COSE 검증으로 교체, 등록 시 공개키 저장.
  검증: 실제 브라우저 어서션(또는 고정 테스트벡터)으로 서명검증 PASS/FAIL.
